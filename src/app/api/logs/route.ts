...
export async function POST(req: NextRequest) {
  // Shared-secret check — skip if FIREWALL_INTERNAL_KEY is not configured (local dev)
  const expectedKey = process.env.FIREWALL_INTERNAL_KEY;
  if (expectedKey && req.headers.get("x-firewall-key") !== expectedKey) {
